...
The template includes `sqlServer`, `mecm`, `adk`, `adkWinPe`, and `odbcDriver18`. For each source, provide:
...
before a provider mutation. The complete object, hash, collection, and
assignment gates are enforced by the Marker stage.
...
Resolve every item in `Missing` before proceeding. Preflight confirms the required SQL Server and MECM license acknowledgements and that each has a configured URI, vault location, or cached file. The full MECM stage also requires the ADK, Windows PE add-on, and ODBC Driver 18 entries.
